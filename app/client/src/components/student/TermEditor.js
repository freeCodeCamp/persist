...
class TermEditor extends Component {
    ...
    componentWillMount() {
        ...
        this.graduationTypeOptions = ['2 year', '4 year'].map(option => <MenuItem value={option} key={option} primaryText={option} />);
    }
    ...
    render() {
        ...
        if (termStart) {
            ...
            ['Fall ' + year, 'Winter ' + year, 'Spring ' + year, 'Summer ' + year].forEach(name => {
                termNamesHTML.push(<MenuItem value={name} primaryText={name} key={name} />);
            });
        }
        const nameDisplay = termStart && termEnd && (!termName || termName.length < 1) ? 'block' : 'none';
        ...
        return (
            <form>
                <Col style={{ minHeight: 100 }} xs={12} sm={6} md={6} lg={6}>
                    <ReduxFormGroup form={this} initValue={initialValues.college} field={termKeysObj.college} />
                    {syncErrors && syncErrors.college ? <div style={styles.error}>{syncErrors.college}</div> : null}
                </Col>
                <Col style={{ minHeight: 100 }} xs={12} sm={6} md={6} lg={6}>
                    <ReduxFormGroup form={this} initValue={initialValues.enrolBegin} field={termKeysObj.enrolBegin} />
                </Col>
                ...
                <Col style={{ minHeight: 100 }} xs={12} sm={6} md={6} lg={6}>
                    <Field name="status" component={SelectField} hintText="Status" floatingLabelText="Status">
                        {this.termStatusOptions}
                    </Field>
                </Col>
                <Clearfix visibleSmBlock visibleMdBlock visibleLgBlock />
                <Col style={{ minHeight: 100 }} xs={12} sm={6} md={6} lg={6}>
                    <Field name="creditsEarned" component={TextField} floatingLabelText="Credits Earned" />
                </Col>
                <Col style={{ minHeight: 100 }} xs={12} sm={6} md={6} lg={6}>
                    <Field name="creditsAttempted" component={TextField} floatingLabelText="Credits Attempted" />
                </Col>
                <Clearfix visibleSmBlock visibleMdBlock visibleLgBlock />
                <Col style={{ minHeight: 100 }} xs={12} sm={6} md={6} lg={6}>
                    <Field name="gpa" component={TextField} floatingLabelText="Term GPA" />
                </Col>
                <Col style={{ minHeight: 100 }} xs={12} sm={6} md={6} lg={6}>
                    <Field
                        name="degreeTitle"
                ...
                <Col style={{ minHeight: 100, display: nameDisplay }} xs={12} sm={6} md={6} lg={6}>
                    <Field name="name" component={SelectField} hintText="Name" floatingLabelText="Name">
                        {termNamesHTML}
                    </Field>
                </Col>
                <Col style={{ minHeight: 100 }} xs={12} sm={6} md={6} lg={6}>
                    <Field name="graduationType" component={SelectField} hintText="Graduation Type" floatingLabelText="Graduation Type">
