...
class CaseNotes extends Component {
    ...
    deleteCaseNote() {
        this.props.deleteCaseNote(this.osis, this.caseNote._id).then(() => this.handleClose());
    }
    ...
    saveCaseNote(oldCaseNote, newCaseNote) {
        ...
        this.props.saveCaseNote(caseNote).then(() => {
            this.handleClose();
    ...
    renderDeleteDialog() {
        ...
        const actions = [
            ...
            <FlatButton label="Delete" disabled={spinner} primary={true} onTouchTap={() => this.deleteCaseNote()} />
        ];
        return (
