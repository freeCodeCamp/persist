...
class HeaderNotifications extends Component {
    ...
    render() {
        ...
        const notificationList = notifications.map((notification, iterator) => {
            return <NotificationItem key={iterator} lastAllRead={lastAllRead} notification={notification} />;
        });
        ...
        return (
            <li className="dropdown notifications-menu">
                <a href="#" className="dropdown-toggle" data-toggle="dropdown">
                    ...
                    {unread > 0 ? <span className="label label-warning">{unread}</span> : null}
                </a>
                <ul className="dropdown-menu">
                    <li className="header">
                        <span>You have {unread} notifications</span>
                        <span style={{ cursor: 'pointer', color: '#3c8dbc' }} className="pull-right" onClick={e => this.markAllRead(e)}>
                            Mark all read
                        </span>
                    </li>
                    <li>
